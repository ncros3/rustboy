    ...
    ($target: ident, $self:ident.$instruction:ident) => {{
        match $target {
            ...
            ArithmeticTarget::HL => ({
                ...
                let value = $self.peripheral.read_peripheral(address);
                let new_value = $self.$instruction(value);
    ...
    ($target: ident, $self:ident.$instruction:ident) => {{
        match $target {
            ...
            IncDecTarget::HL => ({
                ...
                let value = $self.peripheral.read_peripheral(address);
                let new_value = $self.$instruction(value);
                $self.peripheral.write_peripheral(address, new_value);
                // compute next PC value
                // modulo operation to avoid overflowing effects
                $self.pc.wrapping_add(1)
            }, RUN_3_CYCLES),
...
macro_rules! load_in_memory {
    ($input_register: ident, $self:ident) => {{
        let address = $self.registers.read_hl();
        let value = $self.registers.$input_register;
        $self.peripheral.write_peripheral(address, value);
        // compute next PC value
        // modulo operation to avoid overflowing effects
        $self.pc.wrapping_add(1)
    }};
    ...
    ($input_register: ident, $self:ident) => {{
        match $input_register {
            ...
            ArithmeticTarget::D8 => ({
                ...
                let value = $self.peripheral.read_peripheral(value_address);
                let mem_address = $self.registers.read_hl();
                $self.peripheral.write_peripheral(mem_address, value);
                // compute next PC value
                // modulo operation to avoid overflowing effects
                $self.pc.wrapping_add(2)
            }, RUN_3_CYCLES),
    ...
    ($register: ident, $self:ident) => {{
        match $register {
            U16Target::BC => {
                ...
                let low_byte = $self.peripheral.read_peripheral(low_address);
                let high_byte = $self.peripheral.read_peripheral(high_address);
...
impl Cpu {
    ...
    fn debug_run(&self) {
        ...
        if let Some(break_point) = self.break_point {
            if break_point == self.pc {
                ...
                println!("instruction byte : {:#04x} / pc : {:#06x} / sp : {:#04x}", self.peripheral.read_peripheral(self.pc), self.pc, self.sp);
    ...
    fn decode(&mut self, instruction_byte: u8) -> Option<Instruction> {
        if Instruction::is_long_instruction(instruction_byte) {
            let long_instruction_byte = self.peripheral.read_peripheral(self.pc.wrapping_add(1));
    ...
    pub fn run(&mut self) -> u8 {
        ...
        match self.mode {
            ...
            CpuMode::RUN => {
                // fetch instruction
                let instruction_byte = self.peripheral.read_peripheral(self.pc);
                // decode instruction
    ...
    fn load_sp(&mut self, target: SPTarget) -> (u16, u8) {
        match target {
            SPTarget::FROM_SP => ({
                let low_byte_address = self.peripheral.read_peripheral(self.pc.wrapping_add(1)) as u16;
                let high_byte_address = self.peripheral.read_peripheral(self.pc.wrapping_add(2)) as u16;
                let address = low_byte_address + (high_byte_address << 8);

                // save Stack Pointer lower byte
                let mut data = (self.sp & 0x00FF) as u8;
                self.peripheral.write_peripheral(address, data);
                // save Stack Pointer higher byte
                data = ((self.sp & 0xFF00) >> 8) as u8;
                self.peripheral.write_peripheral(address + 1, data);
            ...
            SPTarget::TO_HL => ({
                let address = self.pc.wrapping_add(1);
                let value = self.peripheral.read_peripheral(address) as i8 as i16 as u16;
    ...
    fn load_store_ram(&mut self, target: RamTarget, load: bool) -> (u16, u8) {
        match target {
            RamTarget::OneByteAddress => ({
                ...
                let ram_offset = self.peripheral.read_peripheral(immediate_address) as u16;

                if load {
                    // read data from ram memory & load it in register a
                    self.registers.a = self.peripheral.read_peripheral(base_ram_address + ram_offset);
                } else {
                    // read data from register A & store it in RAM
                    self.peripheral
                        .write_peripheral(base_ram_address + ram_offset, self.registers.a);
                }
            ...
            RamTarget::TwoBytesAddress => ({
                ...
                if load {
                    // read data from ram memory & load it in register a
                    self.registers.a = self.peripheral.read_peripheral(address);
                } else {
                    // read data from register A & store it in RAM
                    self.peripheral.write_peripheral(address, self.registers.a);
                }
    ...
    fn jump_relative(&mut self, flag: bool) -> (u16, u8) {
        ...
        let immediate_address = self.pc.wrapping_add(1);
        let immediate = self.peripheral.read_peripheral(immediate_address) as i8;
    ...
    fn jump_immediate(&mut self, flag: bool) -> (u16, u8) {
        ...
        let low_immediate = self.peripheral.read_peripheral(first_immediate_address);
        let second_immediate_address = self.pc.wrapping_add(2);
        let high_immediate = self.peripheral.read_peripheral(second_immediate_address);
    ...
    fn pop(&mut self) -> u16 {
        ...
        let low_byte = self.peripheral.read_peripheral(low_stack_address) as u16;
        let high_byte = self.peripheral.read_peripheral(high_stack_address) as u16;
    ...
    fn push(&mut self, push_data: u16) {
        ...
        // save data in memory
        self.peripheral.write_peripheral(high_stack_address, high_byte);
        self.peripheral.write_peripheral(low_stack_address, low_byte);
        // update stack pointer
    ...
    fn call(&mut self, flag: bool) -> (u16, u8) {
        ...
        let low_byte_address = self.peripheral.read_peripheral(self.pc.wrapping_add(1));
        let high_byte_address = self.peripheral.read_peripheral(self.pc.wrapping_add(2));
...
mod cpu_tests {
    ...
    fn test_add_memory() {
        ...
        cpu.peripheral.write_peripheral(address, data);
    ...
    fn test_inc_registers() {
        ...
        assert_eq!(cpu.peripheral.read_peripheral(address), 0xAB);
    }
    ...
    fn test_dec_registers() {
        ...
        assert_eq!(cpu.peripheral.read_peripheral(address), 0xA9);
    }
    ...
    fn test_load_registers() {
        ...
        let mut data = 0x23;
        cpu.peripheral.write_peripheral(mem_address, data);
        ...
        assert_eq!(cpu.peripheral.read_peripheral(mem_address), 0x23);

        mem_address = 0x002A;
        cpu.registers.write_hl(mem_address);
        cpu.registers.write_de(0xD500);
        cpu.execute(LOAD(IncDecTarget::HL, D));
        assert_eq!(cpu.peripheral.read_peripheral(mem_address), 0xD5);
    ...
    fn test_load_immediate() {
        ...
        cpu.peripheral.write_peripheral(0x0001, low_data);
        cpu.peripheral.write_peripheral(0x0002, high_data);
    ...
    fn test_store_indirect() {
        ...
        assert_eq!(cpu.peripheral.read_peripheral(mem_address), 0x56);

        data = 0xC600;
        cpu.registers.write_af(data);
        cpu.registers.write_hl(mem_address);
        cpu.execute(STORE_INDIRECT(Load16Target::HL_minus));
        assert_eq!(cpu.peripheral.read_peripheral(mem_address), 0xC6);
    ...
    fn test_jump_relative_nzero() {
        ...
        let mut index = 0;
        for data in program {
            cpu.peripheral.write_peripheral(base_address + index, data);
            index += 1;
        }
        ...
        cpu.run();
        assert_eq!(
            cpu.peripheral.read_peripheral(cpu.pc),
            cpu.peripheral.read_peripheral(base_address + (jump as u16))
        );
        ...
        cpu.pc = base_address;
        cpu.run();
        assert_eq!(cpu.peripheral.read_peripheral(cpu.pc), cpu.peripheral.read_peripheral(base_address + 2));
    }
    ...
    fn test_jump_immediate_zero() {
        ...
        cpu.pc = base_address;
        cpu.run();
        assert_eq!(cpu.peripheral.read_peripheral(cpu.pc), cpu.peripheral.read_peripheral(base_address + 3));
    }
    ...
    fn test_jump_indirect() {
        ...
        let mut index = 0;
        for data in program {
            cpu.peripheral.write_peripheral(index, data);
            index += 1;
        }
        ...
        // run CPU to do the jump
        cpu.run();
        assert_eq!(cpu.peripheral.read_peripheral(cpu.pc), cpu.peripheral.read_peripheral(jump));
    }
    ...
    fn test_load_from_sp_to_mem() {
        ...
        let mut index = 0;
        for data in program {
            cpu.peripheral.write_peripheral(index + base_address, data);
            index += 1;
        }
        ...
        cpu.sp = 0x57A8;
        cpu.run();
        assert_eq!((cpu.sp & 0x00FF) as u8, cpu.peripheral.read_peripheral(address));
        assert_eq!(
            ((cpu.sp & 0xFF00) >> 8) as u8,
            cpu.peripheral.read_peripheral(address + 1)
        );
    }
    ...
    fn test_load_ram_from_one_byte() {
        ...
        let data = 0xF8;
        cpu.peripheral.write_peripheral(ram_data_address, data);
        ...
        let mut index = 0;
        for data in program {
            cpu.peripheral.write_peripheral(index + base_program_address, data);
            index += 1;
        }
        ...
        cpu.run();
        assert_eq!(cpu.registers.a, cpu.peripheral.read_peripheral(ram_data_address));
    }
    ...
    fn test_push_and_pop() {
        ...
        assert_eq!(
            cpu.peripheral.read_peripheral(ram_address.wrapping_sub(1)),
            ((push_data & 0xFF00) >> 8) as u8
        );
        assert_eq!(
            cpu.peripheral.read_peripheral(ram_address.wrapping_sub(2)),
    ...
    fn test_long_rotate_hl() {
        ...
        assert_eq!(cpu.peripheral.read_peripheral(address), 0x6B);
    }
    ...
    fn test_shift_right_and_reset_hl() {
        ...
        assert_eq!(cpu.peripheral.read_peripheral(address), 0x5A);
    }
    ...
    fn test_set_reset_bit_hl() {
        ...
        assert_eq!(cpu.peripheral.read_peripheral(address), 0xB1);

        cpu.execute(Instruction::SET_BIT(BitTarget::BIT_3, IncDecTarget::HL));
        assert_eq!(cpu.peripheral.read_peripheral(address), 0xB9);
    }
}